...
import { processSubmission, evaluateSubmission, generateTrace, getAcceptedSubmissions } from './submission.service';
...
export async function traceCode(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const trace = await (process.env.MOCK_TRACE === 'true') ? [] : await generateTrace(sourceCode);
...
export async function getHistory(req: Request, res: Response, next: NextFunction) {
  try {
    const userId = req.user!.userId;
    const history = await getAcceptedSubmissions(userId);
